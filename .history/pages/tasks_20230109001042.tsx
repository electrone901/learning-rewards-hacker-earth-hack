...
function Explore() {
  ...
  function isSubscribed(task, account) {
    const subscriptions = contract.getSubscriptions[account]
  ...
  function handleClick(task) {
    const subscription = isSubscribed(task, account)
    if (!subscription) {
      contract.subscribe(task.id - 1)
    }  else {
